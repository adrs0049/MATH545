...
However, notice that $\langle \boldsymbol{u}_1 , \boldsymbol{u}_2 \rangle = 1$ which does not equal $0$. If we want to use the projection formula to find the closest vector, the basis needs to be orthonormal. To find the orthogonal basis, we can use the Gram-Schmidt process.
